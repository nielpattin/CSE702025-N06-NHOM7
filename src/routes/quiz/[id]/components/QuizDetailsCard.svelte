				...
				<div>
					...
					<div class="mb-4">
						<div class="mb-2 flex items-center gap-3">
							<Heading tag="h1" class="text-3xl font-bold text-white">
						...
						{#if data.quiz.creator}
							<P class="text-gray-400">by {data.quiz.creator.name}</P>
						{/if}
					</div>
					...
					{#if data.quiz.description}
						<div class="mb-6">
							<Heading tag="h3" class="mb-2 text-lg font-semibold text-white">Description</Heading>
							<P class="text-gray-300">{data.quiz.description}</P>
						</div>
					{/if}
				</div>
