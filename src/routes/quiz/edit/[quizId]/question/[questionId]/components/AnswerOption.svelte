...
<div class="rounded-lg border border-gray-600 bg-gray-700 p-4 shadow-md">
	<div class="flex items-center gap-4">
		<div class="flex-1">
			<label for="option-{index}" class="mb-2 block text-sm font-medium text-gray-300">
				Option {index + 1}
			</label>
			<input id="option-{index}" type="text" value={option.content} oninput={(e) => onContentChange(e.currentTarget.value)} class="w-full rounded-lg border border-gray-600 bg-gray-800 px-3 py-2 text-white placeholder-gray-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="Enter answer content..." />
		</div>
		<div class="flex flex-col items-center gap-2">
			<span class="text-sm font-medium text-gray-300">Correct</span>
			<input type="checkbox" checked={option.correct} onchange={(e) => onCorrectChange(e.currentTarget.checked)} class="h-5 w-5 rounded border-gray-600 bg-gray-700 text-blue-600 focus:ring-blue-500 focus:ring-offset-gray-800" />
		</div>
